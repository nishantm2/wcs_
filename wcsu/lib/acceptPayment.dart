...
import 'package:wcsu/acceptAmount.dart';
  ...
  submit(rfid) async {
    ...
    if (rfid.length == 10) {
      var response = await http.post(
          "http://wcshcst.xyz/acceptPayment.php?rfid=${rfid}");
      if (response.statusCode == 200) {
        ...
        if (jsonResponse != null) {
          setState(() {
            ...
            data = jsonResponse['Customer_ID'];
            sharedPreferences.setString("customerID", data);
            print(data);
            if (data!=null) {
              Navigator.push(
                context,
                MaterialPageRoute(
                  builder: (context) => AcceptAmount(),
                ),
              ).then((value) {
            ...
            showDialog(
                ...
                builder: (BuildContext context) {
                  return RichAlertDialog(
                    alertTitle: richTitle("Alert!"),
                    alertSubtitle: richSubtitle("Try Aagain!"),
                    alertType: RichAlertType.WARNING,
                  );
                });
           }
